      ...
      <div className="max-w-[68rem] mx-auto px-[60px] max-lg:px-6 max-md:px-4">
        <div ref={headerRef} className="text-center max-w-3xl mx-auto mb-12 fade-in-view">
          ...
          <h2 className="text-stone font-bold leading-[1.3] tracking-tight mb-5" style={{ fontSize: 'clamp(2.2rem, 4vw, 2.6rem)' }}>
            Education Insurance Across All 50 States
          </h2>
          <p className="text-cream font-light" style={{ lineHeight: '22px' }}>
            State-specific requirements, compulsory education laws, and coverage guidance — wherever your institution operates.
          </p>
        </div>
        ...
        <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10 stagger-children">
          {featuredStates.map((state) => (
            <Link
              ...
              className="group border-2 border-ash/20 rounded-[2rem] p-5 text-center no-underline hover:border-gold/40"
            ...
            {allStates.map((state) => {
              ...
              return (
                <Link
                  key={slug}
                  href={`/states/${slug}/`}
                  className="text-cream/60 hover:text-gold no-underline"
                  style={{ fontSize: '0.8rem', transition: 'color 0.2s', padding: '0.2rem 0.4rem' }}
                >
                  {state}
                </Link>
              );
        ...
        <div className="mt-14 text-center">
          <h3 className="text-stone font-bold mb-6" style={{ fontSize: '1.3rem' }}>Major Education Markets</h3>
          <div className="flex flex-wrap justify-center gap-2">
            {cities.map((c) => (
              <Link
                key={c.slug}
                href={`/cities/${c.slug}/`}
                className="border border-ash/20 rounded-[2rem] px-4 py-1.5 text-cream/60 hover:text-gold hover:border-gold/30 no-underline"
                style={{ fontSize: '0.75rem', transition: 'all 0.24s' }}
              >
